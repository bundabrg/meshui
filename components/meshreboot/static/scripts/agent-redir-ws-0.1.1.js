...
var CreateAgentRedirect = function (meshserver, module, serverPublicNamePort, authCookie, rauthCookie, domainUrl) {
    ...
    obj.Start = function (nodeid) {
        var url2, url = urlbase?urlbase:(window.location.protocol.replace('http', 'ws') + '//' + window.location.host) + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) + '/' + obj.urlname + '?browser=1&p=' + obj.protocol + (nodeid?('&nodeid=' + nodeid):'') + '&id=' + obj.tunnelid;
