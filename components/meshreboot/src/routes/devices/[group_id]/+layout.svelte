...
<div class="flex flex-col flex-grow-1 p-3 min-w-[500px] bg-white border-e border-e-slate-300">
    ...
    <div class="overflow-auto flex-grow-1">
        <Table class="mt-5" hoverable={true}>
            <TableBody>
                {#each mc.meshes[page.params.group_id]?.nodes as nodeId (nodeId)}
                    {@const node = mc.nodes[nodeId]}
                    {@const node_sessions = Object.keys(node.sessions?.kvm??{}).length}
                    <TableBodyRow>
                        ...
                        <TableBodyCell class="w-full">
                            ...
                            <span>{node.desc || node.osdesc}</span><br>
                            <span>User: {node.users?.join(', ')}</span>
                        </TableBodyCell>
                        <TableBodyCell class="text-gray-300">
                            ...
                            <Icon size="3em" src="{ComputerDesktop}" theme="solid"
                                  class={['inline', node.conn && 'text-green-800']}/>
                        </TableBodyCell>
                    </TableBodyRow>
                {/each}
            </TableBody>
